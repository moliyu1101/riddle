...
class SettingsServiceTests(StateResetMixin, unittest.TestCase):
    ...
    def test_editing_task_persists_backup_models(self) -> None:
        task = SimpleNamespace(
            model_config_json={
                "inherit_global": False,
                "base_url": "https://multi.invalid/v1",
                "api_key": f"{FAKE_SECRET}-multi",
                "model": "model-primary",
                "protocol": "openai_chat",
            },
            fofa_query="",
        ...
        request = UpdateTaskRequest(model_config_data=PartialModelConfigDTO(
            ...
            base_url="https://multi.invalid/v1",
            model="model-primary",
            models=["model-backup-1", "model-backup-2"],
            protocol="openai_chat",
        ))
        runtime_config = LLMConfig(
            base_url="https://multi.invalid/v1",
        ...
        self.assertEqual(result, {"ok": True})
        self.assertEqual(
            task.model_config_json["models"],
            ["model-backup-1", "model-backup-2"],
        )

    def test_editing_task_clears_backup_models_with_empty_list(self) -> None:
        task = SimpleNamespace(
            model_config_json={
                "inherit_global": False,
                "base_url": "https://multi.invalid/v1",
        ...
        self.assertEqual(result, {"ok": True})
        self.assertEqual(task.model_config_json["models"], [])
    ...
    # —— 单端点多模型灾备：同供应商多模型展开 + 失败自动顶替 ——
    def test_normalize_model_list_supports_multiple_formats(self) -> None:
        cases = [
            (["a", "b", "a", "", "c"], ["a", "b", "c"]),
            ('["a", "b", "a"]', ["a", "b"]),
            ("a, b, a, c", ["a", "b", "c"]),
        ...
        for value, expected in cases:
            with self.subTest(value=value):
                self.assertEqual(settings_service._normalize_model_list(value), expected)

    def test_single_endpoint_expands_models_into_same_provider_pool(self) -> None:
        with patch.object(
            settings_service,
            "effective_settings",
            return_value={"llm": {
                ...
                "api_key": f"{FAKE_SECRET}-multi",
                "model": "model-primary",
                "models": ["model-primary", "model-backup-1", "model-backup-2"],
                "protocol": "openai_chat",
                "temperature": 0.3,
                "providers": [],
            }},
        ...
        self.assertEqual(
            ...
            ["model-primary", "model-backup-1", "model-backup-2"],
        )
        self.assertEqual(providers[0].weight, 10)
        self.assertEqual(providers[1].weight, 1)
        self.assertEqual(providers[2].weight, 1)
        self.assertEqual(providers[0].base_url, "https://multi.invalid/v1")
        self.assertEqual(providers[0].api_key, providers[1].api_key)
        self.assertTrue(all(provider.enabled for provider in providers))

    def test_single_endpoint_without_models_keeps_single_provider(self) -> None:
        with patch.object(
            settings_service,
            "effective_settings",
            return_value={"llm": {
        ...
        self.assertEqual(len(providers), 1)
        self.assertEqual(providers[0].model, "model-only")
        self.assertEqual(providers[0].weight, 10)

    def test_task_models_override_global_models(self) -> None:
        task = SimpleNamespace(model_config_json={
            "inherit_global": False,
            "base_url": "https://task.invalid/v1",
            "api_key": f"{FAKE_SECRET}-task",
            "model": "task-primary",
            "models": ["task-primary", "task-backup"],
            "protocol": "openai_chat",
        })
        with patch.object(
            settings_service,
            ...
            return_value={"llm": {
                ...
                "api_key": f"{FAKE_SECRET}-global",
                "model": "global-model",
                "models": ["global-model", "global-backup"],
                "protocol": "openai_chat",
                "temperature": 0.3,
                "providers": [],
            }},
        ...
        self.assertEqual(
            [provider.model for provider in providers],
            ["task-primary", "task-backup"],
        )
        self.assertEqual(providers[0].base_url, "https://task.invalid/v1")

    def test_llm_client_for_task_enables_pool_mode_for_multi_model(self) -> None:
        task = SimpleNamespace(id="task-multi", model_config_json={
            "inherit_global": False,
            "base_url": "https://multi.invalid/v1",
            "api_key": f"{FAKE_SECRET}-multi",
            "model": "model-primary",
        ...
        with patch.object(
            ...
            llm = settings_service.llm_client_for_task(task)

        self.assertTrue(llm.pool_mode)
        self.assertEqual(len(llm.providers), 2)
        self.assertEqual(llm.providers[0].model, "model-primary")
        self.assertEqual(llm.providers[1].model, "model-backup")

    def test_update_settings_persists_normalized_models(self) -> None:
        row = SimpleNamespace(
            llm={},
            fofa={},
            engines={},
        ...
        payload = {
            "llm": {
                ...
                "api_key": f"{FAKE_SECRET}-multi",
                "model": "model-primary",
                "models": ["model-primary", "model-backup", "model-primary", ""],
                "protocol": "openai_chat",
            }
        }
        ...
        async def run_update():
            with patch.object(
                ...
                with patch.object(settings_service, "refresh_cache", new=AsyncMock()):
                    return await settings_service.update_settings(session, payload)
        ...
        self.assertIsInstance(result["llm"]["models"], list)

    def test_public_settings_view_returns_models(self) -> None:
        with patch.object(
            settings_service,
            "effective_settings",
            return_value={
        ...
        self.assertEqual(view["llm"]["models"], ["model-primary", "model-backup"])
        self.assertEqual(view["llm"]["model"], "model-primary")

    def test_create_task_persists_models(self) -> None:
        request = CreateTaskRequest(
            name="multi-model-client",
            model_config_data=ModelConfigDTO(
                base_url="https://multi.invalid/v1",
                api_key=f"{FAKE_SECRET}-multi",
                model="model-primary",
        ...
        self.assertEqual(result, {"ok": True})
        self.assertEqual(
            created_task.model_config_json["models"],
            ["model-primary", "model-backup"],
        )
        self.assertEqual(created_task.model_config_json["model"], "model-primary")

    def test_llm_settings_dto_accepts_models(self) -> None:
        dto = LLMSettingsDTO(
            mode="single",
            base_url="https://multi.invalid/v1",
            model="model-primary",
        ...
        self.assertEqual(dto.models, ["model-primary", "model-backup"])
        self.assertEqual(dto.model, "model-primary")

    def test_probe_task_models_resolves_task_level_key(self) -> None:
        task = SimpleNamespace(
            model_config_json={
                "base_url": "https://task.invalid/v1",
                "api_key": f"{FAKE_SECRET}-task",
        ...
        async def fake_list_models(**kwargs):
            ...
            return {"ok": True, "error": "", "models": ["model-task", "model-other"]}

        body = SimpleNamespace(
            base_url="https://task.invalid/v1",
            api_key="",
            key_ref=settings_service.secret_ref(f"{FAKE_SECRET}-task"),
            protocol="openai_chat",
        )
        ...
        async def run_probe():
            with (
                patch.object(settings_service, "effective_settings", return_value={
                    ...
                    "fofa": {}, "engines": {}, "defaults": {}, "auth": {}, "ui": {},
                }),
                patch.object(tasks_api, "list_available_models", new=fake_list_models),
            ):
                return await tasks_api.probe_task_models("task-1", body, session)

        import asyncio
        result = asyncio.run(run_probe())

        self.assertEqual(result["ok"], True)
        self.assertEqual(captured["api_key"], f"{FAKE_SECRET}-task")
        self.assertEqual(captured["base_url"], "https://task.invalid/v1")

    def test_test_llm_with_task_id_resolves_task_level_key(self) -> None:
        task = SimpleNamespace(
            model_config_json={
                "base_url": "https://task.invalid/v1",
                "api_key": f"{FAKE_SECRET}-task",
        ...
        def fake_test_configs(body):
            captured["api_key"] = body.api_key
            captured["base_url"] = body.base_url
        ...
        async def fake_test_one(name, provider):
            return {"ok": True, "latency_ms": 12, "tool_calling": "yes", "error": ""}

        body = settings_api.LLMTestRequest(
            base_url="https://task.invalid/v1",
            api_key="",
            key_ref=settings_service.secret_ref(f"{FAKE_SECRET}-task"),
            model="model-task",
        ...
        async def run_test():
            with (
                ...
                patch.object(settings_api, "refresh_cache", new=AsyncMock()),
                patch.object(settings_api, "_test_configs", new=fake_test_configs),
                patch.object(settings_api, "_test_llm_one", new=fake_test_one),
            ):
                return await settings_api.test_llm(body, session)
...
class LLMClientPoolTests(StateResetMixin, unittest.TestCase):
    ...
    def test_single_endpoint_multi_model_failover_switches_to_backup(self) -> None:
        # 单端点多模型灾备：同 base_url / api_key，仅 model 不同，主模型失败自动顶替
        primary = LLMConfig(
            base_url="https://multi.invalid/v1",
            api_key=f"{FAKE_SECRET}-multi",
            model="model-primary",
        ...
        expected = object()
        first_error = client_module.LLMError("network", "mock primary model failure")

        with patch.object(client_module.LLMClient, "_build_client", return_value=Mock()):
            llm = client_module.LLMClient(providers=[primary, backup])
            with (
                patch.object(llm, "_provider_order", return_value=[primary, backup]),
                patch.object(
                    llm,
                    "_chat_current_provider",
                    side_effect=[first_error, expected],
        ...
        self.assertIs(result, expected)
        self.assertEqual(invoke.call_count, 2)
        self.assertEqual(llm.config.model, backup.model)
        self.assertEqual(llm.config.base_url, primary.base_url)
        self.assertEqual(llm.config.api_key, primary.api_key)
    ...
    def test_pool_mode_retries_once_before_failover(self) -> None:
        """池模式对瞬时错误（network）软重试一次再切换，避免网络毛刺丢上下文。"""
        ...
        # 每个端点各软重试 1 次（_POOL_SAME_PROVIDER_RETRIES=1），不无限重试
        self.assertEqual(first_client.chat.completions.create.call_count, 2)
        self.assertEqual(second_client.chat.completions.create.call_count, 2)
