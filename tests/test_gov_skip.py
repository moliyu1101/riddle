...
class SensitiveSkipTests(unittest.TestCase):
    ...
    def test_extra_env_suffix(self):
        old = os.environ.get("RIDDLE_SENSITIVE_HOSTS")
        try:
            os.environ["RIDDLE_SENSITIVE_HOSTS"] = "sensitive.test,blocked.example"
            ...
            if old is None:
                os.environ.pop("RIDDLE_SENSITIVE_HOSTS", None)
            else:
                os.environ["RIDDLE_SENSITIVE_HOSTS"] = old
