...
from app.security import set_db_tokens
...
_cache: dict[str, Any] = {"llm": {}, "fofa": {}, "engines": {}, "defaults": {}, "ui": {}, "auth": {}}
...
def _normalize_model_list(value: Any) -> list[str]:
    """把 models 配置归一成去重、去空的模型名列表（支持 JSON 数组 / 逗号分隔 / 单个字符串）。"""
    if isinstance(value, list):
        raw = value
    else:
...
def _env_llm() -> dict[str, Any]:
    ...
    return {
        ...
        "models": _normalize_model_list(os.environ.get("LLM_MODELS_JSON", "")),
...
def _env_auth() -> dict[str, Any]:
    """环境变量令牌（DB 未设置时的兜底来源）。"""
    return {
        "full_token": os.environ.get("RIDDLE_API_TOKEN", "").strip(),
        "read_token": os.environ.get("RIDDLE_READ_TOKEN", "").strip(),
        "observer_token": os.environ.get("RIDDLE_OBSERVER_TOKEN", "").strip(),
    }
...
def effective_settings() -> dict[str, Any]:
    ...
    return {
        ...
        "auth": _merge_section(_cache.get("auth"), _env_auth()),
...
def resolve_llm_providers(task: Task | None = None) -> list[LLMConfig]:
    ...
    # 单端点多模型灾备：models = 主模型 + 灾备模型，展开成「同供应商多模型池」。
    # 主模型 weight 高优先，其余 weight 低作灾备；运行时 failover 自动顶替。
    # 灾备列表里若混入主模型名则剔除，避免池里出现重复模型。
    raw_models = _normalize_model_list(mc.get("models") or eff.get("models") or [])
    models = [model] + [m for m in raw_models if m and m != model]
    configs = []
...
def public_settings_view() -> dict[str, Any]:
    ...
    return {
        "llm": {
            ...
            "model": llm["model"],
            "models": _normalize_model_list(llm.get("models") or [llm.get("model")]),
        ...
        "auth": {
            "full_token": mask_secret(auth.get("full_token")),
            "full_token_set": bool(auth.get("full_token")),
            "read_token": mask_secret(auth.get("read_token")),
            "read_token_set": bool(auth.get("read_token")),
            "observer_token": mask_secret(auth.get("observer_token")),
            "observer_token_set": bool(auth.get("observer_token")),
            "env_full": bool(os.environ.get("RIDDLE_API_TOKEN", "").strip()),
            "env_read": bool(os.environ.get("RIDDLE_READ_TOKEN", "").strip()),
            "env_observer": bool(os.environ.get("RIDDLE_OBSERVER_TOKEN", "").strip()),
        },
...
async def refresh_cache(session: AsyncSession) -> SystemSettings:
    ...
    # 同步 DB 令牌到 security.py（优先于环境变量），保证设置页修改即时生效
    set_db_tokens(
        full=_cache["auth"].get("full_token", ""),
        read=_cache["auth"].get("read_token", ""),
        observer=_cache["auth"].get("observer_token", ""),
    )
    return row
...
def export_settings() -> dict[str, Any]:
    """导出完整配置（含明文密钥），供管理员备份/迁移。"""
    eff = effective_settings()
    llm = eff["llm"]
    fofa = eff["fofa"] or {}
    return {
        "version": 1,
        "exported_at": _cache.get("updated_at"),
        "llm": {
            "mode": normalize_llm_mode(llm.get("mode")),
...
async def update_settings(session: AsyncSession, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if "llm" in payload and payload["llm"]:
        ...
        for k, v in llm_update.items():
            ...
            if k == "providers":
                ...
                continue
            if k == "models":
                llm["models"] = _normalize_model_list(v)
                continue
            if k == "mode":
    ...
    if "auth" in payload and payload["auth"] is not None:
        auth = dict(row.auth or {})
        incoming = dict(payload["auth"] or {})
        for k, v in incoming.items():
            if k not in ("full_token", "read_token", "observer_token"):
                continue
            val = str(v or "").strip()
            if is_masked_secret(val):
                continue  # 前端回显的脱敏占位，保持原值
            if val:
                auth[k] = val
            else:
                auth.pop(k, None)  # 空串 = 清除该令牌
        row.auth = auth
...
def llm_client_for_task(
    ...
    providers = resolve_llm_providers(task)
    # 单端点多模型灾备：providers>1 时按池模式调度，失败自动切到同供应商其它模型。
    pool_mode = resolve_llm_runtime_mode(task) == "pool" or len(providers) > 1
    return LLMClient(
        providers=providers,
        pool_mode=pool_mode,
