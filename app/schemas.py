...
class Evidence(BaseModel):
    ...
    snapshot_ref: Optional[str] = Field(None, description="capture_evidence 保存的存证快照引用（evidence/snap_*.json），提交时自动合并进报告证据链")
    snapshot: Optional[dict] = Field(None, description="自动存证快照（服务端抓取目标页面 HTTP 快照后写入，无需手动填）")
