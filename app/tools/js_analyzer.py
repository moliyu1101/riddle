...
# ===== 业务状态机信号：多步业务流（向导/审批/支付/预约/注册激活） =====
# 全部线性正则，无嵌套量词/交替回溯（历史事故根因 #5 已整改）。
_STEP_VAR_RE = re.compile(
    r"""(?i)\b(?:currentStep|stepIndex|stepNo|stepNum|currentStage|currentPhase|flowStep|wizardStep|activeStep|step)\b"""
)
_STEP_FN_RE = re.compile(
    r"""(?i)\b(?:nextStep|prevStep|goToStep|setStep|changeStep|updateStep|finishStep|submitStep|confirmStep|completeStep|toStep)\b"""
)
_STATE_TRANSITION_RE = re.compile(
    r"""(?i)\b(?:updateStatus|changeStatus|setStatus|updateState|changeState|setState|statusChange|stateChange|transitionTo|advanceTo|moveTo)\b"""
)
_FLOW_TOKEN_RE = re.compile(
    r"""(?i)\b(?:stepToken|flowToken|flowId|processId|processInstanceId|instanceId|businessKey|orderToken|verifyToken|applyNo|applyId|approvalId)\b"""
)
_WIZARD_COMPONENT_RE = re.compile(
    r"""(?i)\b(?:el-steps|a-steps|ant-steps|form-wizard|FormWizard|Wizard|Stepper|ProgressSteps|StepForm|Steps)\b"""
)
_STATUS_ENUM_RE = re.compile(
    r"""(?i)(?:待审批|已审批|已通过|已驳回|未支付|已支付|待支付|待发货|已发货|待收货|已完成|待审核|审核通过|审核驳回|待激活|已激活|待确认|已确认|待受理|已受理)"""
)
# 状态机信号最少命中数：低于此数视为噪声，不产出 finding。
...
def analyze_javascript(text: str, *, base_url: str = "", source: str = "inline") -> dict[str, Any]:
    ...
    findings.extend(_find_state_machines(normalized, base_url, source))
    ...
    return {
        ...
        "state_machines": _state_machine_map(findings),
...
def _find_state_machines(text: str, base_url: str, source: str) -> list[JsFinding]:
    """识别业务状态机/多步流信号（向导/审批/支付/预约/注册激活）。

    业务逻辑洞（步骤跳过/状态篡改/令牌重放/并发竞态）是通用扫描器挖不到的
    差异化来源；前端 JS 里的 step/status/flowToken 信号是发现这些流的第一步。
    只做信号聚合，不执行目标 JS。
    """
    signals: dict[str, set[str]] = {
        ...
        "flow_tokens": set(),
        "wizard_components": set(),
        "status_values": set(),
    }
    for pat, key, limit in (
        (_STEP_VAR_RE, "step_vars", 200),
        (_STEP_FN_RE, "transition_fns", 200),
        (_STATE_TRANSITION_RE, "state_transitions", 200),
        (_FLOW_TOKEN_RE, "flow_tokens", 200),
        (_WIZARD_COMPONENT_RE, "wizard_components", 100),
        (_STATUS_ENUM_RE, "status_values", 200),
    ):
        for m in _iter_matches(pat, text, limit=limit):
            signals[key].add(m.group(0).strip())

    # 只保留有区分度的 step 变量（裸 step 太常见，仅作辅助信号）
    if "step" in signals["step_vars"] and len(signals["step_vars"]) > 1:
        signals["step_vars"].discard("step")

    active = {k: v for k, v in signals.items() if v}
    if len(active) < _STATE_MACHINE_MIN_SIGNALS:
        return []

    # 命中强度：信号类型越多越可信
    strength = len(active)
    score = 55 + strength * 6
    severity = "high" if strength >= 4 else "medium" if strength >= 3 else "info"
    evidence_parts = []
    for k in ("step_vars", "transition_fns", "state_transitions", "flow_tokens", "wizard_components", "status_values"):
        if signals[k]:
            evidence_parts.append(f"{k}:{'/'.join(sorted(signals[k])[:6])}")
    evidence = "; ".join(evidence_parts)

    next_steps = [
        "识别多步流的步骤边界：列出 step/status/flowId 参数与对应接口，构造基线请求",
        "步骤跳过：直接请求第 N 步接口，看是否校验前置步骤完成（无 step token/完成标记校验）",
        "状态篡改：把 status/state 参数从待审批改成已通过、未支付改成已支付，看后端是否校验状态机迁移合法性",
        "步骤令牌：删除/复用 stepToken/flowId/processId，看是否仍放行",
        "并发竞态：同一资源并发提交，看是否绕过名额/数量/重复领取限制",
    ]
    findings = [JsFinding(
        "state_machine",
        "业务状态机/多步流信号",
        ...
        score,
        evidence,
        _context(text, max(0, text.lower().find("step")), 160),
        _absolute_url("", base_url),
        source,
        ["state_machine", "business_logic", "logic_flaw"],
        next_steps,
    )]
    return findings


def _state_machine_map(findings: list[JsFinding]) -> list[dict[str, Any]]:
    """把状态机 finding 聚合成结构化 map，供 worker 直接读取。"""
    out: list[dict[str, Any]] = []
    for f in findings:
        if f.kind != "state_machine":
            continue
        signals: dict[str, list[str]] = {}
        for part in (f.evidence or "").split(";"):
            part = part.strip()
            if ":" not in part:
                continue
            key, _, values = part.partition(":")
            signals[key] = [v for v in values.split("/") if v]
        out.append({
            "label": f.title,
...
def _build_chains(findings: list[JsFinding], base_url: str) -> list[JsChain]:
    ...
    if any(f.kind == "state_machine" for f in findings):
        sm = next(f for f in findings if f.kind == "state_machine")
        chains.append(JsChain(
            "business_state_machine",
            "业务状态机/多步流信号，可验证步骤跳过与状态篡改",
            sm.severity,
            "高危候选" if sm.severity == "high" else "中危候选",
            "前端存在多步业务流信号（step/status/flowToken/向导组件），命中「步骤跳过/状态篡改/令牌重放」业务逻辑洞链路。",
            [sm.evidence],
            sm.next_steps,
        ))
    return chains
...
def _fetch_text(url: str, *, timeout: float) -> tuple[str, str]:
    ...
    req = Request(url, headers={"User-Agent": "Mozilla/5.0 (compatible; Riddle-JSAnalyzer)"})
...
def _fetch_local_file(url: str) -> tuple[str, str]:
    ...
    allowed_roots = [
        ...
        (Path.home() / ".cache" / "riddle").resolve(),
    ]
