...
def extract_route_signals(
    *,
    ...
    if business_id:
        _add_signal(signals, f"biz:{business_id}", business_id)
...
def route_target(
    *,
    ...
    for route in _ROUTES:
        ...
        # 业务画像命中：业务逻辑测试是差异化核心，给业务/IDOR 与 API 授权路线加权。
        if business_id and route.route_id in ("upload_business_idor", "api_authorization"):
            score += 2.0
            tags.append(f"biz:{business_id}")
            evidence.append(f"业务画像:{business_id}")
        if not tags:
            continue
