    ...
    if task.target_source in ("manual", "both") and task.manual_targets:
        ...
        for item in parsed:
            ...
            if prefilter.is_sensitive_host(host) or prefilter.is_sensitive_host(url):
                ...
                continue
            pending.append({"url": url or _ensure_url(host), "host": host, "note": item.get("note") or ""})
        if pending:
            await progress(
                "dispatch",
...
async def _site_collect(
    ...
    for item in parsed:
        ...
        if prefilter.is_sensitive_host(host) or prefilter.is_sensitive_host(url):
            ...
            continue
        work.append({"url": url or _ensure_url(host), "host": host, "note": item.get("note") or ""})
    ...
    for c in pending:
        ...
        for route in c.get("missing_routes") or routes:
            reason = site_collab.route_reason(route)
            if note:
                reason = f"{note}；{reason}"[:300]
            session.add(Target(
                task_id=task.id,
                url=url,
                host=host,
