...
def clean_manual_target_lines(raw: list[str] | str) -> list[str]:
    """供创建/编辑任务落库：保留行尾业务备注的清理行列表。

    过滤注释行/空行/非目标行，但保留目标行及其备注（如「https://x 教务系统」），
    供 collector 重新解析出 note 驱动业务画像/状态机/差异化策略。
    """
    lines = [ln for ln in _split_lines(raw) if ln and not ln.startswith("#")]
    return [ln for ln in lines if _line_produces_target(ln)]


def _line_produces_target(line: str) -> bool:
    """该行能否解析出至少一个目标（预览里用来标记忽略行）。"""
    if _PAREN_IP_LINE.match(line):
        return True
    m_trail = _TRAILING_PAREN_IP.search(line)
    token_part = line[: m_trail.start()].rstrip() if m_trail else line
    m = _TOKEN_NOTE.match(token_part)
    if not m:
        return False
    token = m.group(1)
    if not _looks_like_target_token(token):
...
def preview_manual_targets(raw: list[str] | str, limit: int = 50) -> dict:
    """供创建/编辑任务前端实时预览：解析统计 + 忽略行 + 目标样本（不落库）。"""
    lines = [ln for ln in _split_lines(raw) if ln and not ln.startswith("#")]
    entries = parse_manual_targets(raw)
    hosts = [e["host"] for e in entries]
    domains = sum(1 for h in hosts if not _IPV4.match(h))
    ignored = [ln for ln in lines if not _line_produces_target(ln)]
    return {
        "total_lines": len(lines),
        "valid": len(entries),
        "domains": domains,
        "ips": len(hosts) - domains,
        "with_note": sum(1 for e in entries if e.get("note")),
        "ignored": ignored[:20],
        "ignored_total": len(ignored),
        "targets": [
            {"url": e["url"], "host": e["host"], "note": e.get("note", "")}
