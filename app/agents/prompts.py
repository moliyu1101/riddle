...
- **闭环恢复纪律（重要）**：测试过程中凡是改变了状态的操作，测完必须恢复或确保不影响后续判断：
  · access_boundary 测权限边界会临时清空会话——工具内部已用快照-恢复保护，但你换接口再测时要确认当前会话态正确（登录态/匿名态别搞混）。
  · credential_brute/login_session 失败后会话自动恢复原态；命中后保持新登录态——后续测接口时要清楚当前是哪个账号的登录态，别用 A 账号登录态去测 B 账号的越权。
  · upload_probe 成功上传后会留一个 test.txt 在目标上无法自动删除——提交相关漏洞时在 evidence.notes 说明此副作用。
  · 任何你手动 session_set 改过的会话态，测完如果不再需要，用 session_set(clear=True) 清掉，避免后续请求带错误 cookie 误判。
  · 原则：测前记住当前状态→测完恢复或确认新状态→再做下一步，避免状态串号导致误报/漏报。
"""
...
- capture_evidence: 新工具，存证快照：对确认漏洞的页面 URL（含 PoC 参数）抓取结构化 HTTP 快照（状态码/响应头/正文片段/标题/可见文本/耗时）作为存证，保存后返回 evidence_ref。命中漏洞后、submit_finding 前调用，把 evidence_ref 通过 submit_finding 的 evidence.snapshot_ref 带上，报告证据链会自动合并该存证。只读为主、走当前会话。注意：即使不手动调用，submit_finding 提交时也会自动对 target_url 抓一份存证快照；手动调用主要用于存证带 PoC 参数的具体页面。
