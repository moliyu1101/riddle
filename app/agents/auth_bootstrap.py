...
_COMMENT_PREFIXES = ("#", "//", ";")
_HTTP_METHOD_LINE = re.compile(r"^\s*(?:GET|POST|PUT|DELETE|HEAD|OPTIONS|PATCH|CONNECT|TRACE)\s+\S+", re.I)
_AT_CRED = re.compile(r"^\s*(?:https?://)?([^:/@\s]+):([^@\s]+)@(\S+)\s*$")


def _split_cred_line(line: str) -> Optional[dict[str, str]]:
    """把一行结构化凭据拆成 {target, username, password}；拆不出返回 None。"""
    if _HTTP_METHOD_LINE.match(line) or " HTTP/" in line:
        return None  # 原始 HTTP 请求行，不是凭据
    m = _AT_CRED.match(line)  # user:pass@host（可带 https:// 前缀）
    if m:
        return {"target": m.group(3), "username": m.group(1), "password": m.group(2)}
    if "|" in line:  # host|user|pass 或 user|pass
        parts = [p.strip() for p in line.split("|")]
        if len(parts) == 3 and parts[1] and parts[2]:
            return {"target": parts[0], "username": parts[1], "password": parts[2]}
        if len(parts) == 2 and parts[0] and parts[1]:
            return {"target": "", "username": parts[0], "password": parts[1]}
    parts = line.split()
    if len(parts) == 3 and parts[0] and parts[1] and parts[2]:  # host user pass
        return {"target": parts[0], "username": parts[1], "password": parts[2]}
    if "://" in line:
        return None  # 纯 URL，不是凭据
    for sep in (":", "/", ","):  # user:pass / user/pass / user,pass
        if sep in line:
            p = [x.strip() for x in line.split(sep)]
            if len(p) == 2 and p[0] and p[1]:
                if p[1].isdigit() and ("." in p[0] or p[0].lower() == "localhost"):
                    continue  # host:port，不是凭据
                return {"target": "", "username": p[0], "password": p[1]}
            if len(p) > 2 and p[0] and all(x for x in p[1:]):
                return {"target": "", "username": p[0], "password": sep.join(p[1:])}
    if len(parts) == 2 and parts[0] and parts[1]:  # user pass
        return {"target": "", "username": parts[0], "password": parts[1]}
    return None


def _parse_auth_line(line: str) -> Optional[dict[str, Any]]:
    """单行 → 一条 binding（raw 形式）。头格式/中文账密整行交给 normalize_binding 解析。"""
    if _AUTH_HEADER_RE.search(line) or _COOKIE_HEADER_RE.search(line) \
            or _BEARER_RE.search(line) or _USER_PASS_LINE.search(line):
        return {"raw": line}
    if _KV_COOKIE.match(line.split(";")[0].strip()):  # 裸 Cookie 串
        return {"raw": line}
    return _split_cred_line(line)


def parse_auth_batch(text: str) -> list[dict[str, Any]]:
    """把批量文本拆成多条凭据绑定（已 normalize，带 kinds）。

    支持每行一条：
    - 头格式：Cookie: ... / Authorization: Bearer ... / 账号: ... 密码: ...
    - 结构格式：user:pass / user/pass / user,pass / user pass
    - 带目标：host|user|pass / user:pass@host / host user pass
    - 注释行（#、//、; 开头）与空行忽略
    """
    ...
    for raw_line in (text or "").splitlines():
        ...
        if not parsed:
            continue
        nb = normalize_binding(parsed)
        if nb["kinds"]:
            out.append(nb)
    return out


def preview_auth_batch(text: str, limit: int = 500) -> dict[str, Any]:
    """供创建/编辑任务前端批量导入实时预览：统计 + 绑定样本 + 忽略行（不落库）。"""
    lines = [ln.strip() for ln in (text or "").splitlines()]
    lines = [ln for ln in lines if ln and not ln.startswith(_COMMENT_PREFIXES)]
    bindings = parse_auth_batch(text)
    ignored = [ln for ln in lines if _parse_auth_line(ln) is None]
