...
def _extra_sensitive_suffixes() -> tuple[str, ...]:
    """环境变量 RIDDLE_SENSITIVE_HOSTS：逗号分隔额外后缀/完整域名。"""
    raw = os.environ.get("RIDDLE_SENSITIVE_HOSTS", "") or ""
    out = []
...
def is_sensitive_host(host_or_url: str) -> bool:
    ...
    - 环境变量 RIDDLE_SENSITIVE_HOSTS 追加的后缀
    """
    ...
    try:
        with capped_resolution():
            with httpx.Client(timeout=timeout, verify=False, follow_redirects=True) as c:
                r = c.get(url, headers={"User-Agent": "Mozilla/5.0 (compatible; Riddle)"})
                body = r.text or ""
