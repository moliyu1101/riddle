...
def to_cst(dt: datetime) -> datetime:
    """UTC naive（或带 tz）时间统一转东八区 datetime，供分桶/聚合使用。"""
...
class Finding(Base):
    ...
    id: Mapped[str] = mapped_column(String(32), primary_key=True, default=_uuid)
    # task_id/target_id 可空：任务删除时过审漏洞孤立化保留（脱离级联删除）
    task_id: Mapped[str | None] = mapped_column(ForeignKey("tasks.id"), index=True, nullable=True)
    target_id: Mapped[str | None] = mapped_column(ForeignKey("targets.id"), index=True, nullable=True)
...
class Review(Base):
    ...
    # task_id 可空：任务删除时过审漏洞的审核记录随漏洞保留（孤立化）
    task_id: Mapped[str | None] = mapped_column(ForeignKey("tasks.id"), index=True, nullable=True)
...
class Killsweep(Base):
    ...
    # 出洞闭环回写：该通杀产品实际产出的 finding 关联
    # [{finding_id, title, school, host, status, severity, created_at}]
    derived_findings: Mapped[list] = mapped_column(JSON, default=list)
    # 分析进度：{stage: fingerprint|fofa|verify|done, label, pct, ts}
    progress: Mapped[dict] = mapped_column(JSON, default=dict)
    # 失败原因分类：llm_error / no_key / timeout / cancelled / other
    fail_reason: Mapped[str] = mapped_column(String(30), default="")
...
class SystemSettings(Base):
    ...
    auth: Mapped[dict] = mapped_column(JSON, default=dict)      # 访问令牌：full_token/read_token/observer_token
    updated_at: Mapped[datetime] = mapped_column(DateTime, default=_now, onupdate=_now)


class ReportVersion(Base):
    """漏洞报告版本历史：每次人工编辑/助手改稿落一份快照，可回看、可回滚。

    snapshot 保存该版本生效的报告字段（title/description/affected_scope/steps/poc/
    severity/user_notes），与 Review.user_edits 同构；version 从 1 起按 finding 递增。
    """
    __tablename__ = "report_versions"
    __table_args__ = (
        # 同一 finding 内版本号唯一（回滚按 version 定位）
        Index("ux_report_versions_finding_version", "finding_id", "version", unique=True),
        # 列表按时间倒序取
        Index("ix_report_versions_finding_created", "finding_id", "created_at"),
    )
    ...
    source: Mapped[str] = mapped_column(String(20), default="user_edit")  # user_edit/assistant/review/system
