...
_MIGRATIONS = [
    ...
    ("killsweeps", "fail_reason", "VARCHAR(30) DEFAULT ''"),
    ...
    ("system_settings", "auth", "JSON DEFAULT '{}'"),
    ("tasks", "guard_ops", "JSON DEFAULT '[]'"),
]
...
# 需要改为可空的列：任务删除时过审漏洞孤立化保留（置空 task/target 外键脱离级联删除）。
# SQLite 不支持 ALTER COLUMN 改约束，用重建表的方式迁移（保留数据/索引/FK）。
# (table, [列名])
_NULLABLE_COLUMN_MIGRATIONS = [
    ("findings", ["task_id", "target_id"]),
    ("reviews", ["task_id"]),
]
...
async def init_db() -> None:
    ...
    # 列改可空需重建表，且 SQLite 的 DROP TABLE 在有子表外键引用时会失败
    # （如 reviews 引用 findings.id），而 PRAGMA foreign_keys 在事务内是 no-op，
    # 因此放到事务外、关闭外键约束的独立连接上执行。
    await _migrate_nullable_columns()
    # 索引必须在表重建之后补建（重建会连带删除原表索引）
    async with engine.begin() as conn:
...
async def _migrate_nullable_columns() -> None:
    """列改可空（任务删除时保留过审漏洞需要）。

    SQLite 的 DROP TABLE 在 foreign_keys=ON 且有子表外键引用时会失败
    （如 reviews 引用 findings.id），而 PRAGMA foreign_keys 在事务内是 no-op，
    因此用独立连接 + AUTOCOMMIT 关闭外键后重建表。
    """
    if not _NULLABLE_COLUMN_MIGRATIONS:
        return
    async with engine.connect() as conn:
        await conn.execution_options(isolation_level="AUTOCOMMIT")
        await conn.exec_driver_sql("PRAGMA foreign_keys=OFF;")
        try:
            for table, cols in _NULLABLE_COLUMN_MIGRATIONS:
                await _recreate_table_nullable(conn, table, cols)
        finally:
            await conn.exec_driver_sql("PRAGMA foreign_keys=ON;")


async def _recreate_table_nullable(conn, table: str, cols_to_null: list[str]) -> None:
    """SQLite 不支持 ALTER COLUMN 改约束；重建表让指定列可空（保留数据/索引/FK）。

    幂等：目标列已全部可空时跳过。重建后索引由 _ensure_unique_indexes /
    _ensure_secondary_indexes 统一补建。
    """
    ...
    col_notnull = {r[1]: r[3] for r in info}
    if all(col_notnull.get(c, 0) == 0 for c in cols_to_null):
        return  # 已全部可空
    sql = row[0]
    for col in cols_to_null:
        # 去掉该列定义里的 NOT NULL（只匹配列定义行，不影响 FK/索引里的列名）
        sql = re.sub(
    ...
    new_sql = re.sub(
        ...
        rf"\1\2{table}__new\3", sql, count=1, flags=re.IGNORECASE,
    )
    col_names = ", ".join(f'"{c[1]}"' for c in info)
    await conn.exec_driver_sql(new_sql)
    await conn.exec_driver_sql(
        f'INSERT INTO "{table}__new" ({col_names}) SELECT {col_names} FROM "{table}"'
    )
    await conn.exec_driver_sql(f'DROP TABLE "{table}"')
    await conn.exec_driver_sql(f'ALTER TABLE "{table}__new" RENAME TO "{table}"')
