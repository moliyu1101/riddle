...
- RIDDLE_READ_TOKEN：只读（GET + WebSocket 看板，禁止一切写操作）
- RIDDLE_OBSERVER_TOKEN：观摩（仅安全概览，禁止敏感信息与写操作）
"""
...
_TOKEN_ENV = "RIDDLE_API_TOKEN"
_READ_TOKEN_ENV = "RIDDLE_READ_TOKEN"
_OBSERVER_TOKEN_ENV = "RIDDLE_OBSERVER_TOKEN"
_AUTH_EXEMPT_PATHS = {"/api/auth/status", "/health", "/favicon.svg", "/favicon.ico"}

# 设置页可自定义的令牌（DB 持久化，优先于环境变量）。由 settings_service 在
# 启动与每次配置更新时刷新；环境变量仍是兜底来源。
_db_tokens: dict[str, str] = {"full": "", "read": "", "observer": ""}


def set_db_tokens(full: str = "", read: str = "", observer: str = "") -> None:
    _db_tokens["full"] = str(full or "").strip()
    _db_tokens["read"] = str(read or "").strip()
    _db_tokens["observer"] = str(observer or "").strip()
...
def configured_full_token() -> str:
    return _db_tokens["full"] or os.environ.get(_TOKEN_ENV, "").strip()


def configured_read_token() -> str:
    return _db_tokens["read"] or os.environ.get(_READ_TOKEN_ENV, "").strip()


def configured_observer_token() -> str:
    return _db_tokens["observer"] or os.environ.get(_OBSERVER_TOKEN_ENV, "").strip()
