...
logger = logging.getLogger("riddle.llm")
...
# 部分模型/网关不支持原生 function calling。为了让这类模型也能跑知蠹 Riddle 的
...
def _resolve_tool_compat(config: Any) -> str:
    ...
    v = (
        ...
        or os.environ.get("RIDDLE_TOOL_COMPAT")
...
class LLMClient:
    ...
    def force_prompt_tools_current(self) -> bool:
        ...
        时主动自愈：不必等用户手动设 RIDDLE_TOOL_COMPAT=prompt。
